...
func intercept(ast: PackedStringArray) -> bool:
	...
	print("Intercept: "+cmd_name)
	if cmd_name.begins_with("+"):
		print("Pressing " + cmd_name.substr(1))
		Input.action_press(cmd_name.substr(1))
		return true;
	if cmd_name.begins_with("-"): 
		print("Releasing " + cmd_name.substr(1))
		Input.action_release(cmd_name.substr(1))
		return true;
	match cmd_name:
		"greet": __cmd_greet()
